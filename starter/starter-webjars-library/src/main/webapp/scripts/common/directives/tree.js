define("common/directives/tree", ["angular", "$directives"], function (angular, $directives) {
    // angular-tree-control.js
    $directives.directive('tree', ['$compile', function ($compile) {
            /**
            ...
            return {
                ...
                controller: ['$scope', function ($scope) {
                    ...
                    $scope.headClass = function (node, $isLast) {
                        var liSelectionClass = classIfDefined($scope.options.injectClasses.liSelected, false);
                        var injectSelectionClass = "", value = "";
                        if (liSelectionClass && isSelectedNode(node))
                            injectSelectionClass = " " + liSelectionClass;
                        if ($scope.options.isLeaf(node)) {
                            value = "tree-leaf " + injectSelectionClass + ($isLast ? " tree-leaf-last" : "");
                        } else if ($scope.expandedNodesMap[this.$id]) {
                            value = "tree-expanded" + injectSelectionClass + ($isLast ? " tree-expanded-last" : "");
                        } else {
                            value = "tree-collapsed" + injectSelectionClass + ($isLast ? " tree-collapsed-last" : "");
                        }
                        return value
                    };
                    ...
                    var template =
                        ...
                        '<li  ng-repeat="node in node.' + $scope.options.nodeChildren + ' | filter:filterExpression:filterComparator ' + orderBy + '" ' +
                        'ng-class="headClass(node,$last)" ' + classIfDefined($scope.options.injectClasses.li, true) + '>' +
                        '<div class="title" ng-click="selectNodeHead(node)">' +
                        ...
                        '</div>' +
                        '<div class="items" tree-item ng-if="nodeExpanded()" ></div>' +
                        '</li>' +
                        '</ul>';
...
//                        });
                        ...
                        treemodelCntr.template(scope, function (clone) {
                            var rootElement = angular.element('<ul class="tree-root"></ul>');
                            if (options.rootTitle) {
                                rootElement.append('<li class="tree-root-item"><div class="tree-label">'+options.rootTitle+'</div></li>');
                            }
                            rootElement.append(clone)
                            element.html('').append(rootElement);
                        });
